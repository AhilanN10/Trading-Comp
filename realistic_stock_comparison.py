...
def compare_stocks():
    # Test top performers from our scans
    ...
    for symbol in stocks:
        ...
        print(f"  {symbol}: {total_return:+.2f}% (${final_val:.2f})")
    ...
    ax1.barh(df['Stock'], df['Return (%)'], color=colors, alpha=0.7)
    ax1.axvline(x=0, color='black', linestyle='-', linewidth=0.5)
    ax1.set_xlabel('Return (%)', fontsize=12)
    ax1.set_title('Realistic Returns by Stock\n($100 Starting Capital, Random Week)', fontsize=12, fontweight='bold')
    ax1.grid(True, alpha=0.3, axis='x')
    
    # Final value comparison
    ax2.bar(df['Stock'], df['Final ($)'], color='#00C805', alpha=0.7)
    ax2.axhline(y=100, color='gray', linestyle='--', alpha=0.5, label='Starting Capital')
    ...
    plt.tight_layout()
    output_path = '/Users/ahilannayani/.gemini/antigravity/brain/e1722e60-7bf5-4969-922b-a2f9396acbc6/realistic_stock_comparison.png'
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"\nChart saved to: {output_path}")
    
    print("\n" + "="*80)
    print("REALISTIC STOCK COMPARISON ($100 Starting Capital)")
    print("="*80)
